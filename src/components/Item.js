...
class Item extends React.Component {
    ...
    render() {
        ...
        return (
            <div className={"item" + (this.props.item.mastered ? " item-mastered" : "") + ((this.props.item.minMR || 0) > this.props.mr ? " item-locked" : "")} >
                {this.state.showTooltip && <Tooltip title="Information" x={this.state.x} y={this.state.y}>
                    <>
                        ...
                        {(() => {
                            ...
                            if (Object.keys(components).length === 0) return <span className="item-uncraftable">UNCRAFTABLE</span>;
                            ...
                            return Object.keys(components).map(item => {
                                return <div key={item}>
                                    ...
                                    <span className="component-name">{components[item].toLocaleString() + "x " + item}</span>
                                    <br />
                                </div>
                            })
                        })()}
                        ...
                        {(this.props.item.buildTime && this.props.item.buildTime > 60) &&
                            <>
                                ...
                                })()} - {<img src={credits} alt="" width="15px"></img>} {this.props.item.buildPrice.toLocaleString()}{this.props.item.minMR ? " - Mastery Rank " + this.props.item.minMR : ""}</span>
                                <br />
                            </>
                        }
                        <br />
